-- Apaga View
drop MATERIALIZED VIEW mvwestoque2025 ;

-- Cria View
create materialized view mvwestoque2025  as
    select
	p.codproduto
	, p.produto
	, p.inativo
	...
	inner join tblestoquemes em on (em.codestoquemes = (select em2.codestoquemes from tblestoquemes em2 where em2.codestoquesaldo = es.codestoquesaldo and em2.mes <= '2025-12-31' order by mes desc limit 1))
...
where e.codproduto in (
	...
	from mvwestoque2025 e2
	where e2.quant < 0
)
group by codproduto, produto
order by sum(preco * quant) 
--order by abs(sum(preco * quant)) desc
...
select ' google-chrome https://sistema.mgpapelaria.com.br/MGLara/produto/' || codproduto::varchar, *
from mvwestoque2025  e
where e.codproduto in (
	select e2.codproduto
	from mvwestoque2025 e2
...
--where produto ilike '%papel%a4%500%'
...
select codfilial, codproduto, produto, preco, quant, custo, valor, markup 
from mvwestoque2025
where codfilial = 105
order by 7 desc


select * from tblestoquemovimento where codestoquemes = 5646647 order by entradaquantidade  desc
