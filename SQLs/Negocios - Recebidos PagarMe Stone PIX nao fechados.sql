select f.filial, u.usuario, 'PagarMe', count(*), sum(n.valortotal), max(n.codnegocio), max(n.lancamento)
...
union all
select f.filial, u.usuario, 'PIX', count(*), sum(n.valortotal), max(n.codnegocio), max(n.lancamento)
from tblpix p
...
select date_trunc('month', , 'PagarMe', count(*), sum(n.valortotal), max(n.codnegocio), max(n.lancamento)
from tblpagarmepedido pp 
inner join tblnegocio n on (n.codnegocio = pp.codnegocio)
inner join tblfilial f on (f.codfilial = n.codfilial)
inner join tblusuario u on (u.codusuario = n.codusuario)
